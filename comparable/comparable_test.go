...
func Test_Char(t *testing.T) {
	...
	strEqual(t, APart(comp, 1), `b`, `APart(Char, 1)`)
	strEqual(t, BPart(comp, 1), `d`, `BPart(Char, 1)`)
}
...
func Test_Integer(t *testing.T) {
	...
	strEqual(t, APart(comp, 1), `2`, `APart(Integer, 1)`)
	strEqual(t, BPart(comp, 1), `4`, `BPart(Integer, 1)`)
}
...
func Test_Interface_Float(t *testing.T) {
	...
	strEqual(t, APart(comp, 1), `1.23`, `APart(Interface(Float), 1)`)
	strEqual(t, BPart(comp, 1), `1.2356`, `BPart(Interface(Float), 1)`)
}
...
func Test_Interface_Cat(t *testing.T) {
	...
	strEqual(t, APart(comp, 1), `&{mittens}`, `APart(Interface(Cat), 1)`)
	strEqual(t, BPart(comp, 1), `&{kitty}`, `BPart(Interface(Cat), 1)`)
}
...
func Test_Runes(t *testing.T) {
	...
	strEqual(t, APart(comp, 1), `b`, `APart(Runes, 1)`)
	strEqual(t, BPart(comp, 1), `d`, `BPart(Runes, 1)`)
}
...
func Test_RuneSlice(t *testing.T) {
	...
	strEqual(t, APart(comp, 1), `cats`, `APart(RuneSlice, 1)`)
	strEqual(t, BPart(comp, 1), `dog`, `BPart(RuneSlice, 1)`)
}
...
func Test_String(t *testing.T) {
	...
	strEqual(t, APart(comp, 1), `b`, `APart(String, 1)`)
	strEqual(t, BPart(comp, 1), `d`, `BPart(String, 1)`)
}

func Test_Custom_Float(t *testing.T) {
	comp := &Float{
		a:       []float64{1.2345, 1.23, 3.14159},
		b:       []float64{1.235, 1.2356},
		epsilon: 0.001}
	intEqual(t, comp.ALength(), 3, `Float.ALength`)
	intEqual(t, comp.BLength(), 2, `Float.BLength`)
	boolEqual(t, comp.Equals(0, 0), true, `Float.Equals(0, 0)`)
	boolEqual(t, comp.Equals(0, 1), false, `Float.Equals(0, 1)`)
	boolEqual(t, comp.Equals(1, 0), false, `Float.Equals(1, 0)`)
	boolEqual(t, comp.Equals(1, 1), false, `Float.Equals(1, 1)`)
	boolEqual(t, comp.Equals(2, 0), false, `Float.Equals(1, 0)`)
	strEqual(t, APart(comp, 1), `?`, `APart(Float, 1)`)
	strEqual(t, BPart(comp, 1), `?`, `BPart(Float, 1)`)
}
...
func (f *Float) ALength() int {
	return len(f.a)
}

func (f *Float) BLength() int {
	return len(f.b)
}

func (f *Float) Equals(aIndex, bIndex int) bool {
	af, bf := f.a[aIndex], f.b[bIndex]
	if af > bf {
		return af-bf < f.epsilon
	}
	return bf-af < f.epsilon
}
